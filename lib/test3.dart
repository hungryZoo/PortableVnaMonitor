...
List<InterpolationNode> convertArrayToNodes(Array x, Array y) {
  if (x.length != y.length) {
    throw ArgumentError('x and y arrays must have the same length.');
  }

  return List<InterpolationNode>.generate(
      x.length, (i) => InterpolationNode(x: x[i], y: y[i]));
}
...
class TxBaseLine {
  ...
  Array matInterp(Array inArray) {
    var dataPoints =
        convertArrayToNodes(linspace(0, 1, num: inArray.length), inArray);
    var spline = SplineInterpolation(nodes: dataPoints);

    List<double> xValues = linspace(0, 1, num: targetPoint);
    List<double> interpolatedValues =
        xValues.map((x) => spline.compute(x)).toList();
    Array outArray = Array(interpolatedValues);
    return outArray;
  }
...
void main(List<String> arguments) {
  ...
  a.initMatResample((a.numPoint - 1) * 200 + 1);
  ...
  for (var i = 0; i < 1000; i++) {
    ...
    Array zrealInterp = a.matInterp(zrealFilt);
